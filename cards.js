...
{ tasks : function()
{
	var myArray = post.find().fetch();
        return distinctArray = _.uniq(myArray, false, function(d) {return d.matiere});
...
{ tasks : function()
{
	return post.find({matiere:Session.get('matiere')},{sort:{'level':1}});
...
e.preventDefault();
      ...
      {
      var card = {  'matiere':Session.get('matiere'),
		    'question':question,
                    'answer':answer,
		    'level':level};
	//ajout de l'element 
...
'click .level':function(e)
{
	...
	post.update({_id:this._id},{$set:{level:$(e.currentTarget).val()}});
}
}
);
}
